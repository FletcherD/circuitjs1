...
MouseOutHandler, MouseWheelHandler {
	...
	public void setCanvasSize(){
		...
		if (cv != null) {
			...
			cv.setCoordinateSpaceWidth(Math.round(width*zoom));
			cv.setCoordinateSpaceHeight(Math.round(height*zoom));
		}
		if (backcv != null) {
			backcv.setWidth(width + "PX");
			backcv.setHeight(height + "PX");
			backcv.setCoordinateSpaceWidth(Math.round(width*zoom));
			backcv.setCoordinateSpaceHeight(Math.round(height*zoom));
		}
		if (g != null) {
			g.setZoom(zoom);
		}
	...
	public void updateCircuit() {
		...
		g = new Graphics(backcontext, zoom);
