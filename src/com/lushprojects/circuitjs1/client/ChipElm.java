...
abstract class ChipElm extends CircuitElm {
	...
	void drawChip(Graphics g) {
		...
		for (i = 0; i != getPostCount(); i++) {
			...
			if (p.bubble) {
				...
				drawThickCircle(g, p.bubblePos, 1);
				g.setColor(lightGrayColor);
				drawThickCircle(g, p.bubblePos, 3);
			}
		...
		if (clockPoints != null)
			g.drawPolyline(clockPoints);
		for (i = 0; i != getPostCount(); i++)
			drawPost(g, pins[i].post.x, pins[i].post.y, nodes[i]);
		g.setFont(oldfont);
	}
	double rectPointsX[], rectPointsY[];
	Point[] clockPoints;
	Pin pins[];
	int sizeX, sizeY;
	boolean lastClock;
	...
	void setPoints() {
		...
		int xs = sizeX*cspc2;
		int ys = sizeY*cspc2;
		rectPointsX = new double[] { xr, xr+xs, xr+xs, xr };
		rectPointsY = new double[] { yr, yr, yr+ys, yr+ys };
	...
	class Pin {
		...
		void setPoint(int px, int py, int dx, int dy, int dax, int day,
			...
			if (bubble) {
				bubblePos.x = xa+dax*10*csize;
				bubblePos.y = ya+day*10*csize;
			}
			if (clock) {
				Point[] clockPoints = new Point[3];
				clockPoints[0] = new Point(xa+dax*cspc-dx*cspc/2, ya+day*cspc-dy*cspc/2);
				clockPoints[1] = new Point(xa, ya);
				clockPoints[2] = new Point(xa+dax*cspc+dx*cspc/2, ya+day*cspc+dy*cspc/2);
			}
		}
	}
}
